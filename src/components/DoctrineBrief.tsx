    ...
    <section className="border-t-4 border-primary bg-background py-16 lg:py-24 transition-colors duration-300 overflow-hidden">
      <div className="container-editorial">
        ...
        >
          ...
          >
            {status === "success" && (
              <div className="flex items-center gap-3 rounded-lg bg-green-500/20 border border-green-500/30 text-green-400 p-4">
                <CheckCircle2 className="h-5 w-5 flex-shrink-0" />
                <p className="text-sm font-medium">Your brief has been securely transmitted to the editorial team. Thank you.</p>
              </div>
            )}
            {status === "error" && (
              <div className="flex items-center gap-3 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 p-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              ...
              className="mt-4 w-full rounded-md flex items-center justify-center gap-2 bg-[hsl(var(--brand-red))] py-4 text-sm font-bold uppercase tracking-[0.2em] text-white transition-all duration-500 ease-out hover:bg-primary hover:shadow-[0_0_20px_rgba(var(--primary),0.4)] focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-foreground disabled:opacity-70 disabled:cursor-not-allowed"
            >
